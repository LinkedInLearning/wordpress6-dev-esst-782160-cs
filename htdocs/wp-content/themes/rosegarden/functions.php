<?php
/**
 * Rosegarden functions and definitions
 *
...
function rosegarden_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'rosegarden' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'rosegarden_custom_background_args',
			array(
...
function rosegarden_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'rosegarden_content_width', 640 );
}
...
function rosegarden_scripts() {
	wp_enqueue_style( 'rosegarden-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'rosegarden-style', 'rtl', 'replace' );

	wp_enqueue_script( 'rosegarden-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
